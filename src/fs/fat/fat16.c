...
struct fat_directory_item
{
    ...
    uint16_t low_16_bits_last_cluster;
    uint32_t filesize;
} __attribute__((packed));
...
struct fat_private
{
    ...
    // Used to stream file file allocation table
...
{
    // The joins the high and low bits together
    return (item->high_16_bits_first_cluster) | item->low_16_bits_last_cluster;    
}
...
struct fat_directory* fat16_load_fat_directory(struct disk* disk, struct fat_directory_item* item)
{
    ...
    int cluser_sector = fat16_cluster_to_sector(fat_private, cluster);
    int total_items = fat16_get_total_items_for_directory(disk, cluser_sector);
    directory->total = total_items;
...
static void fat16_free_file_directory(struct fat_file_descriptor* desc)
{
...
int fat16_close(void* private)
{
    fat16_free_file_directory((struct fat_file_descriptor*)private);
    return 0;
}
